...
class MLPRegressor:
    ...
    def _backward(self,x,y,A1,A2,A3,Z1,Z2,Z3,W1,W2,W3,B1,B2,B3,lr):
        ...
        dW2 =((-2/n)*(dZ2).T@A1).T
        dW1 =((-2/n)*(dZ1).T@x).T
        dB3 = (-2/n) * np.sum(dZ3, axis=0, keepdims=True)
        dB2 = (-2/n) * np.sum(dZ2, axis=0, keepdims=True)
    ...
    def fit(self,x ,y,lr=0.0001,epochs=100)->None:
...
model = MLPRegressor()
lr=0.01
model.fit(x_train,y_train,lr=lr,epochs=500)
